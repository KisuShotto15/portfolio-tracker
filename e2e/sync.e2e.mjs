...
await sleep(300);
const p24 = JSON.parse(await ev("(function(){var g=document.querySelector('.p2p-grid'),oe=document.getElementById('p2p-out');if(!g||!oe)return '{}';var r=function(e){var b=e.getBoundingClientRect();return {t:Math.round(b.top),l:Math.round(b.left),b:Math.round(b.bottom),w:Math.round(b.width)};};var sell=r(document.getElementById('p2p-sell')),buy=r(document.getElementById('p2p-buy')),fee=r(document.getElementById('p2p-comm')),card=r(document.getElementById('p2p-out'));var t=document.getElementById('tc-p2p').getBoundingClientRect();return JSON.stringify({sellBuyMismaFila:sell.t===buy.t,feeCardMismaFila:Math.abs(fee.t-card.t)<70,feeIzq:fee.l<card.l,cardDer:card.l>fee.l,alineanAbajo:Math.abs(fee.b-card.b)<=2,abajoDeSell:fee.t>sell.b,anchoTool:Math.round(t.height)});})()"));
...
// La celda del spread es la misma caja que el input de al lado: como chip con
// fondo y tamano propios rompia la simetria de la fila.
const sim24 = JSON.parse(await ev("(function(){var r=function(e){var b=e.getBoundingClientRect();return {w:Math.round(b.width),h:Math.round(b.height),t:Math.round(b.top)};};var oe=document.getElementById('p2p-out');if(!oe)return '{\"i\":{},\"o\":\"no existe\",\"lbl2\":false}';var i=r(document.getElementById('p2p-comm')),o=r(oe);var l=[...document.querySelectorAll('#tc-p2p .field-lbl')].map(function(e){return Math.round(e.getBoundingClientRect().top)});return JSON.stringify({i:i,o:o,lbl2:l[2]===l[3]});})()"));
check('la caja del spread mide igual que el input', sim24.i.h > 0 && sim24.i.w === sim24.o.w && sim24.i.h === sim24.o.h && sim24.i.t === sim24.o.t, JSON.stringify(sim24));
check('y sus dos labels van a la misma altura', sim24.lbl2 === true, JSON.stringify(sim24));
...
check('P2P sigue entero', tools24.p2pInp === 3 && /%$/.test(tools24.p2pOut||''), JSON.stringify(tools24));
