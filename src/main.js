...
function renderWallets(){
  ...
  trackerNames.forEach(function(name){
    ...
    var plus5=name==='Zelle'?'<div style="font-size:10px;color:var(--color-text-secondary)">+5%: '+fmtUSD(total*1.05)+'</div>':'';
    cards.push('<div class="wcard"><div class="wcard-name"><span class="wstatus" style="background:#EF9F27"></span>'+escHtml(name)+' <span class="badge-t">tracker</span></div><div style="display:flex;justify-content:space-between;align-items:center"><div class="wcard-bal" style="color:#a78bfa">'+fmtUSD(total)+'</div><div style="display:flex;align-items:center;gap:6px">'+plus5+actions+'</div></div><div style="font-size:11px;color:var(--color-text-secondary);margin-top:4px">Calculated from transactions</div></div>');
  });
  S.manualWallets.filter(function(w){ return !w.trackerOnly; }).forEach(function(w){
    ...
    cards.push('<div class="wcard"><div class="wcard-name"><span class="wstatus" style="background:#EF9F27"></span>'+escHtml(w.name)+' <span style="font-size:10px;color:var(--color-text-secondary)">(manual)</span></div><div style="display:flex;justify-content:space-between;align-items:center"><div class="wcard-bal '+(w.balance<0?'r':'b')+'">'+fmtUSD(w.balance)+'</div>'+actions+'</div></div>');
  });
